...
[进度](https://developer.mozilla.org/zh-CN/docs/Learn/CSS/First_steps/How_CSS_is_structured)
...
3. HTML中标记了类名（class）的元素，可以在CSS中通过`.`+类名选中：
...
4. **包含选择符**可以指定选择器为满足特定嵌套关系的元素（内部），只需要在它们之间加上空格：
...
5. **相邻选择符**可以指定选择器为满足特定相邻关系的元素（后者），只需要在它们之间加上加号：
...
10. CSS中的属性和值都是区分大小写的。

11. CSS中可以使用一些函数，例如`calc()`、`rotate()`。

12. **@规则**可以提供一些特殊的规则，例如导入额外的CSS样式表可以通过`@import 'styles2.css'`；也可以给出一些限制条件，例如：
...
13. 4值`padding`速记属性一般按照**上，右，下，左**的顺序；2值`padding`速记属性先是**上，下**，后是**左，右**：
...
14. 使用速记属性会使未涉及的值被修改为默认值。

15. 当需要一次性设置所有属性值时，可以使用`all`属性。